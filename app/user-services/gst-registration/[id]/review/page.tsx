...
const ReviewGstRegistration = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const regData = await new GstRegistrationService().getById(id);
            const docData = await Promise.all(
                regData.documents
                    .filter((doc) => !!doc)
                    .map(async (doc) => await new DocumentService().getById(doc))
            );
            if (!regData) {
                toast({
                    title: "Error",
            ...
            setDocuments(Array.isArray(docData) ? docData : [docData]);
        } catch (error) {
            console.error("Error fetching data:", error);
            toast({
                title: "Error",
                description: "Failed to fetch registration details. Please try again.",
                variant: "destructive",
            });
        } finally {
            setLoading(false);
    ...
    const handleSubmit = async () => {
        try {
            setSubmitting(true);
            await new GstRegistrationService().submit(id);
            toast({
                title: "Success",
