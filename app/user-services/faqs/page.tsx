...
export default function FAQ() {
    ...
    const faqs = [
        {
            question: 'How does BeFiler work?',
            answer:
                'BeFiler is a powerful tool with a user-friendly interface that simplifies tax filing in Pakistan. Answer simple yes/no questions and provide basic information like salary, taxes paid, and investments. Our system calculates your tax refund or obligation after adjusting taxes already paid. Built-in checks ensure all required information is collected, and our integrated system files your return under tax expert supervision, notifying you upon completion.',
        },
        {
            question: 'Why should I use BeFiler when my tax consultant does the same work?',
            answer:
                'BeFiler educates you on your finances, unlike traditional consultants who file returns without your involvement. Our step-by-step process helps you understand your finances, identify tax-saving opportunities, and apply tax credits. Your data is stored for future reference, reducing paperwork. Contact us to learn more about tax filing in Pakistan.',
        },
        {
            question: 'What do I need to get started?',
            answer:
                'BeFiler combines technology and tax expertise in three steps: Sign up, enter basic information and answer simple questions, and pay using common methods like debit/credit cards.',
        },
        {
        ...
        },
        {
            ...
            answer:
                'BeFiler offers expert support via phone at (021) 38892069 (9:00 a.m. to 5:30 p.m., Monday to Friday) and email at info@befiler.com.',
        },
        {
            question: 'Is my sensitive information secure with BeFiler?',
            answer:
    ...
    return (
        <>
            <Head>
                <title>BeFiler - Frequently Asked Questions</title>
                <meta name="description" content="Answers to common questions about tax filing in Pakistan with BeFiler." />
                <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600;700&display=swap" rel="stylesheet" />
            </Head>
            <div className="min-h-screen bg-white py-16 px-4 sm:px-8 lg:px-12">
                <div className="max-w-5xl mx-auto">
                    <h1 className="text-4xl sm:text-5xl font-bold text-red-600 mb-12 text-center font-poppins tracking-tight">
                    ...
                    <div className="space-y-6">
                        {faqs.map((faq, index) => (
                            ...
                            >
                                <button
                                    className="w-full text-left bg-white text-gray-900 p-6 flex justify-between items-center focus:outline-none hover:text-red-600 transition-colors duration-300"
                                    onClick={() => toggleFAQ(index)}
                                    aria-expanded={activeIndex === index}
                                    aria-controls={`faq-answer-${index}`}
                                >
                                    <span className="text-xl sm:text-2xl font-semibold font-poppins">{faq.question}</span>
                                    <span className="text-2xl font-bold text-red-600">{activeIndex === index ? '−' : '+'}</span>
                                </button>
                                <div
                                    id={`faq-answer-${index}`}
